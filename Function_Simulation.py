...
In this document, a function is provided to do the simulations. '''
...
''' This part creates the simulation, with the following variables:
    
p represents the external field;
h is the strength of the external field;
theta is the strength of the interaction;
Iter is the number of iterations we do;
...
def Simulation(p, h, theta, Iter = 2000, dt = 0.01, X = [], phi = [], phist = True):
    ...
    if phist == True:    
        ...
        plt.xticks([0,np.pi/2,np.pi,3*np.pi/2, 2*np.pi],['0',r'$\frac{\pi}{2}$',r'$\pi$',r'$\frac{3\pi}{2}$',r'$2\pi$'])
        plt.legend(loc=1)
        plt.savefig(f"Histograms/Xfinal_p{p}_h{h}_theta{theta}.pdf",dpi=300,bbox_inches='tight')
        plt.show()
